...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return x * attention_weights  # 重新缩放特征图


# 定义注意力蒸馏损失
def attention_loss(feature_map_s, feature_map_t):
    # 创建通道注意力模块
    channel_attention = ChannelAttention(in_channels=feature_map_s.size(1)).cuda()

    # 生成注意力图
    attention_s = channel_attention(feature_map_s)  # 学生特征图的注意力图
    attention_t = channel_attention(feature_map_t)  # 教师特征图的注意力图
