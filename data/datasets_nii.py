...
class Brats_loadall_nii(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_all = torch.squeeze(torch.from_numpy(mask_array[14]), dim=0)
        return x, yo, mask, mask_all, name
...
class Brats_loadall_val_nii(Dataset):
    def __init__(self, transforms='', root=None, settype='train', modal='all',val_file='test.txt'):
